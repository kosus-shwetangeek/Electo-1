...
{
    public partial class SenateReport : System.Web.UI.Page
    {
        ...
        protected void Drp_Election_SelectedIndexChanged(object sender, EventArgs e)
        {
            ...
            {
                ToNominee = GetSenateNominee(Convert.ToInt32(Drp_Election.SelectedValue));

                ToDesignation = GetSenateDesignation();
        ...
        public List<NomineesEntity> GetSenateNominee(int xiElectionId)
        {
            if (Drp_Election.SelectedValue != "0")
            {
                studentHelper = new UIHelper();
                List<NomineesEntity> lNominees = studentHelper.GetAllSenateNomineesHelp(true).Where(x => x.Nom_ElectionId == xiElectionId).OrderByDescending(x => x.Nom_VoteCount).ToList();
        ...
        public List<DropDownDataSource> GetSenateDesignation()
        {
                DropDownCommon lDropCommon = new DropDownCommon();
                List<DropDownDataSource> lLstRet = lDropCommon.GetSenateDesignationDropDown();
